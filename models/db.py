...
if not request.env.web2py_runtime_gae:
    ...
    db = DAL('mysql://root@localhost/cpfecys', \
           pool_size=1,check_reserved=['all'], migrate=True, fake_migrate_all=True)
        #pool_size=1,check_reserved=['all'], migrate=False)
else:
...
## configure email
#emarquez temporalmente comentado
...
#emarquez - opciones de correo gmail
#mail = auth.settings.mailer
#mail.settings.server = 'smtp@gmail.com:465'
#mail.settings.sender = 'dttsistemas@gmail.com'
##mail.settings.tls = False
#mail.settings.login = 'dttsistemas@gmail.com:guatemala10'
...
db.define_table('course_limit_exception_period',
    Field('project', 'reference project', notnull = True, unique=True, label = T('Course')),
    Field('period_repet', 'boolean', notnull = True, label = T('Repetir periodos variables')),
    Field('period', 'reference period_detail', notnull = True,  label = T('Periodo')),
    Field('date_finish', 'datetime', notnull = True, label = T('Date Finish'))
)
...
db.define_table('evaluation_period',
    Field('date_start', 'date', notnull = True, label = T('Date start')),
    Field('date_finish', 'date', notnull = True, label = T('Date finish')),
    Field('period_repeat', 'boolean', notnull = True, label = T('Repetir cada periodo')),
    Field('description', 'text', notnull=False, label=T('Description')),
    Field('period', 'reference period_detail', notnull = True, label = T('Period')),
    Field('repository_evaluation', 'reference repository_evaluation', notnull = True, label = T('Repository Evaluation'))
)
...
db.define_table('course_report_exception',
    Field('project', 'reference project', notnull = True, unique=True, label = T('Course')),
    Field('period_type', 'reference period_type', notnull = True,  label = T('Tipo Periodo'))
)
...
DSstudents = db((db.auth_user.id ==db.auth_membership.user_id) & (db.auth_membership.group_id == 5)).select(db.auth_user.username, orderby=db.auth_user.username);
...
db.define_table('tesis_status',
                Field('name', 'string', unique = True, notnull=True, required=True, label = T('Name'), length = 255),
                Field('description', 'text', notnull=False,  required=False ,unique=False, label = T('Description')),
                format = '%(name)s')
...
def recursive_parent(FILA):
    DsParent  =  db(db.tesis_research_areas.id == FILA['parent_research_area']).select(db.tesis_research_areas.ALL).first()
    if (DsParent):
        if (DsParent['parent_research_area']):
            retornocadena = '';
            retornocadena =  recursive_parent(DsParent) + retornocadena + '[' + str(FILA['name']) + '] ';
        else:
            retornocadena = '[' + str(DsParent['name']) + '] ' +  '[' + str(FILA['name']) + '] '
    else:
        retornocadena = "";
    return retornocadena

def rsa_represent(parent_research_area, row):
    record = db((db.tesis_research_areas.id == parent_research_area)).select(db.tesis_research_areas.ALL).first()
    if record:
...
db.define_table('tesis_research_areas',
                Field('parent_research_area', 'reference tesis_research_areas', notnull = False, required=False, ondelete='CASCADE', label = T('Categoria Superior')),
                Field('name', 'string', length = 255,  notnull=True, required=True, unique =False, label = T('Name')),
                Field('description', 'text', notnull=False,  required=False ,unique=False, label = T('Description')),
                Field('file1', 'upload', notnull=False, required=False ,unique=False, label=T('Archivo'), requires=[IS_NULL_OR(IS_UPLOAD_FILENAME(extension = '(pdf|zip|rar|doc|docx)',error_message='Solo se aceptan archivos con extension zip|pdf|rar|doc|docx')),IS_LENGTH(2097152,error_message='El tamaño máximo del archivo es 2MB')]),
                format = rsa_format
                )
db.tesis_research_areas.parent_research_area.requires=IS_EMPTY_OR(IS_IN_DB(db, db.tesis_research_areas.id, db.tesis_research_areas._format, zero=T('<Seleccione>')))
db.tesis_research_areas.parent_research_area.represent = rsa_represent

db.define_table('tesis_tutor',
                Field('firstname', 'string', length = 255,  notnull=True, required=True, unique =False, label = T('Nombres')),
                Field('lastname', 'string', length = 255,  notnull=False, required=True, unique =False, label = T('Apellidos')),
                Field('Professionalid', 'integer', length = 15, notnull=False,  required=False ,unique=False, label = T('No. de Colegiado')),
                Field('Profession', 'string', length = 255, notnull=False,  required=False ,unique=False, label = T('Profesión')),
                Field('specialty', 'text', notnull=False,  required=False ,unique=False, label = T('Especialidad')),
                Field('address', 'text', notnull=False,  required=False ,unique=False, label = T('Dirección')),
                Field('phone', 'integer', length = 15,  notnull=False, required=False, unique =False, label = T('Phone')),
                Field('email', 'string', length = 50,  notnull=False, requires = IS_EMAIL(error_message='El email no es valido'), unique =False, label = 'Email'),
                Field('cv', 'upload', notnull=False, required=False ,unique=False, label='Curriculum vitae', requires=[IS_NULL_OR(IS_UPLOAD_FILENAME(extension = '(pdf|zip|rar|doc|docx)',error_message='Solo se aceptan archivos con extension zip|pdf|rar|doc|docx')),IS_LENGTH(2097152,error_message='El tamaño máximo del archivo es 2MB')]),
                Field('file1', 'upload', notnull=False, required=False ,unique=False, label='Adjunto', requires=[IS_NULL_OR(IS_UPLOAD_FILENAME(extension = '(pdf|zip|rar|doc|docx)',error_message='Solo se aceptan archivos con extension zip|pdf|rar|doc|docx')),IS_LENGTH(2097152,error_message='El tamaño máximo del archivo es 2MB')])
...
db.define_table('tesis_asigned_topics',
                Field('student', 'string', length = 50,  notnull=True, required=True, unique =False, label = T('Estudiante')),
                Field('tutor', 'reference tesis_tutor', length = 100,  notnull=False, required=False, unique =False, label = T('Tutor') ),
                Field('adviser', 'reference tesis_adviser', length = 100,  notnull=False, required=False, unique =False, label = T('Asesor')),
                Field('topic_name', 'text', notnull=True,  required=True ,unique=False, label = T('Tema')),
                Field('topic_modality', 'reference tesis_modality', length = 100,  notnull=True, required=True, unique =False, label = T('Modalidad')),
                Field('topic_area', 'reference tesis_research_areas', length = 255,  notnull=True, required=True, unique =False, label = T('Area')),
                Field('topic_description', 'text', notnull=False,  required=False ,unique=False, label = T('Descripción')),
                Field('topic_protocol', 'upload', notnull=False, required=False ,unique=False, label=T('Protocolo'), requires=[IS_NULL_OR(IS_UPLOAD_FILENAME(extension = '(pdf|zip|rar|doc|docx)',error_message='Solo se aceptan archivos con extension zip|pdf|rar|doc|docx')),IS_LENGTH(10485760,error_message='El tamaño máximo del archivo es 10MB')]),
                Field('file1', 'upload', notnull=False, required=False ,unique=False, label='Adjunto', requires=[IS_NULL_OR(IS_UPLOAD_FILENAME(extension = '(pdf|zip|rar|doc|docx)',error_message='Solo se aceptan archivos con extension zip|pdf|rar|doc|docx')),IS_LENGTH(2097152,error_message='El tamaño máximo del archivo es 2MB')]),
                Field('topic_date_approval', 'date', notnull = False, required=False, label = T('Fecha de Aprobación')),
                Field('topic_date_validity', 'date', notnull = False, required=False, label = T('Vigencia')),
                Field('topic_date_reactivation', 'date', notnull = False, required=False, label = T('Fecha de Reactivación')),
                Field('status', 'reference tesis_status', notnull = False, required=True, label = T('Estado')),
                format = '%(student)s - %(topic_name)s')

#se agrego el autocomplete para que muestre solo estudiantes
db.tesis_asigned_topics.student.widget = SQLFORM.widgets.autocomplete(request, db.auth_user.username, db = db((db.auth_user.id ==db.auth_membership.user_id) & (db.auth_membership.group_id == 5)), limitby=(0,5), min_length=4);

db.tesis_asigned_topics.student.requires = [IS_IN_SET(ValueStudents, error_message='Estudiante no existe en base de datos', zero = None)]
...
db.define_table('eps_period_year',
                Field('period', 'reference eps_period', notnull=True, required=True, label = T('Ciclo')),
                Field('yearp', 'integer', notnull=True, required=True, label = T('yearp')),
                Field('start_date', 'date', notnull=True, required=True, label = T('start date')),
                Field('end_date', 'date', notnull=True, required=True, label = T('end date')),
                #format = '%(yearp)s - %(period)s'
                format=lambda r: '%s - %s' % (r.yearp, db.eps_period[r.period].name )
                )
...
db.define_table('eps_activity',
                Field('modality', 'reference eps_modality', notnull=True, required=True, label = T('Modalidad')),
                Field('orden', 'integer', notnull=True, required=True, label = T('Orden')),
                Field ('name', 'string', length = 255,  notnull=True, required=True, unique =True, \
                    label = T('Name')),
                Field('description', 'text', notnull=False,  required=False ,unique=False, label = T('Description')),
                Field('enable_virtual', 'boolean', notnull=False, label = T('Permite entrega digital')),
                Field('file1', 'upload', notnull=False, required=False ,unique=False, label='Archivo 1', requires=[IS_NULL_OR(IS_UPLOAD_FILENAME(extension = '(pdf|zip|rar|doc|docx)',error_message='Solo se aceptan archivos con extension zip|pdf|rar|doc|docx')),IS_LENGTH(2097152,error_message='El tamaño máximo del archivo es 2MB')]),
                Field('file2', 'upload', notnull=False, required=False ,unique=False, label='Archivo 2', requires=[IS_NULL_OR(IS_UPLOAD_FILENAME(extension = '(pdf|zip|rar|doc|docx)',error_message='Solo se aceptan archivos con extension zip|pdf|rar|doc|docx')),IS_LENGTH(2097152,error_message='El tamaño máximo del archivo es 2MB')]),
                Field('file3', 'upload', notnull=False, required=False ,unique=False, label='Archivo 3', requires=[IS_NULL_OR(IS_UPLOAD_FILENAME(extension = '(pdf|zip|rar|doc|docx)',error_message='Solo se aceptan archivos con extension zip|pdf|rar|doc|docx')),IS_LENGTH(2097152,error_message='El tamaño máximo del archivo es 2MB')])
                ,format = '%(name)s')


db.define_table('eps_institution',
                Field('name', 'string', length = 255,  notnull=True, required=True, unique =False, label = T('Name')),
                Field('unit', 'string', length = 255,  notnull=False, required=False, unique =False, label = T('Unidad')),
                Field('description', 'text', notnull=False,  required=False ,unique=False, label = T('Description')),
                Field('address', 'text', notnull=False,  required=False ,unique=False, label = T('Dirección')),
                Field('phone', 'integer', length = 15,  notnull=False, required=False, unique =False, label = T('Phone')),
                Field('email', 'string', length = 50,  notnull=False, requires = IS_EMAIL(error_message='El email no es valido'), unique =False, label = 'Email'),
                Field('owner_name', 'string', length = 255,  notnull=True, required=True, unique =False, label = T('Nombre del Responsable')),
                Field('owner_phone', 'integer', length = 15,  notnull=False, required=False, unique =False, label = T('Telefono del Responsable')),
                Field('owner_email', 'string', length = 50,  notnull=False, requires = IS_EMAIL(error_message='El email no es valido'), unique =False, label = T('Email del Responsable')),
                Field('file1', 'upload', notnull=False, required=False ,unique=False, label='Archivo 1', requires=[IS_NULL_OR(IS_UPLOAD_FILENAME(extension = '(pdf|zip|rar|doc|docx)',error_message='Solo se aceptan archivos con extension zip|pdf|rar|doc|docx')),IS_LENGTH(2097152,error_message='El tamaño máximo del archivo es 2MB')]),
                Field('file2', 'upload', notnull=False, required=False ,unique=False, label='Archivo 2', requires=[IS_NULL_OR(IS_UPLOAD_FILENAME(extension = '(pdf|zip|rar|doc|docx)',error_message='Solo se aceptan archivos con extension zip|pdf|rar|doc|docx')),IS_LENGTH(2097152,error_message='El tamaño máximo del archivo es 2MB')]),
                Field('file3', 'upload', notnull=False, required=False ,unique=False, label='Archivo 3', requires=[IS_NULL_OR(IS_UPLOAD_FILENAME(extension = '(pdf|zip|rar|doc|docx)',error_message='Solo se aceptan archivos con extension zip|pdf|rar|doc|docx')),IS_LENGTH(2097152,error_message='El tamaño máximo del archivo es 2MB')])
                ,format = '[%(name)s] %(unit)s ')


#TABLAS DE FUNCIONAMIENTO DE EPS
db.define_table('eps_assignment',
                Field('status', 'reference eps_status', notnull = False, required=True, label = T('Estado')),
                Field('modality', 'reference eps_modality', notnull = False, required=True, label = T('Modalidad')),
                Field('period', 'reference eps_period_year', notnull = False, required=True, label = T('Periodo')),
                Field('student', 'string', length = 50,  notnull=True, required=True, unique =False, label = T('Estudiante')),
                Field('project', 'text', notnull=True,  required=True ,unique=False, label = T('Proyecto')),
                Field('institution', 'reference eps_institution', notnull=True,  required=True ,unique=False, label = T('Institución')),
                Field('adviser', 'string', length = 255,  notnull=False, required=True, unique =False, label = T('Asesor')),
                Field('supervisor', 'string', length = 255,  notnull=False, required=True, unique =False, label = T('Supervisor')),
                format = '%(student)s - %(project)s')

#se agrego el autocomplete para que muestre solo estudiantes
db.eps_assignment.student.widget = SQLFORM.widgets.autocomplete(request, db.auth_user.username, db = db((db.auth_user.id ==db.auth_membership.user_id) & (db.auth_membership.group_id == 5)), limitby=(0,5), min_length=4);

db.eps_assignment.student.requires = [IS_IN_SET(ValueStudents, error_message='Estudiante no existe en base de datos', zero = None)]

#se agrego el autocomplete para que muestre informacion de institucion
#db.eps_assignment.institution.widget = SQLFORM.widgets.autocomplete(request, db.eps_institution.name, id_field=db.eps_institution.id, limitby=(0,20), min_length=1)

db.define_table('eps_activity_calendar',
                Field('activity', 'reference eps_activity', notnull = True, required=True, label = T('Actividad')),
                Field('period_year', 'reference eps_period_year', notnull = True, required=True, label = T('Periodo')),
                Field('date_assign', 'date', notnull = False, required=True, label = T('Fecha Asignada')),
...
db.define_table('eps_files_enable_virtual',
                Field('activity', 'reference eps_activity', notnull = True, required=True, label = T('Actividad')),
                Field('assign', 'reference eps_assignment', notnull = True, required=True, label = T('Asignacion')),
                Field('files', 'upload', notnull = False, required=False, label = T('Archivo'))
                )
...
db.define_table('eps_tracking_activity',
                Field('activity', 'reference eps_activity', notnull = True, required=True, label = T('Actividad')),
                Field('assign', 'reference eps_assignment', notnull = True, required=True, label = T('Asignacion')),
                Field('validated', 'boolean', notnull=False, required=False, unique=False, label = T('Validado')),
                Field('observations', 'text', notnull=False, required=False, unique=False, label = T('Observaciones'))
                )
