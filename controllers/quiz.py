...
def consult_quiz():
    ...
    a = r.keys(cadena)
    cuestionarios = a
    lista = db().select(db.tb_metadata_quiz.ALL, db.auth_user.first_name, db.auth_user.last_name, db.project.name, join=db.auth_user.on(db.tb_metadata_quiz.creador == db.auth_user.id), join=db.project.on(db.project.id == db.tb_metadata_quiz.curso))
